...
library SolarLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ISolarPair(pairFor(factory, tokenA, tokenB)).getReserves();
...
contract SolarRouter02 is ISolarRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = SolarLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = SolarLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'SolarRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = SolarLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            ISolarPair(SolarLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
...

            {
                ...
                amountOutput = SolarLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SolarLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20Solar(path[path.length - 1]).balanceOf(to);
